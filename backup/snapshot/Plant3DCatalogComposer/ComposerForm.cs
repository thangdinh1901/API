...
namespace Plant3DCatalogComposer
{
    public partial class ComposerForm : UserControl
    {
        ...
        public ComposerForm()
        {
            ...
            _toolTip.SetToolTip(btnGenerateCode, "Export catalog files to catalog_generator/parts on the dev repo (deploy.json)");
            _toolTip.SetToolTip(btnDeployCatalog, "Deploy Python catalog to CustomScripts and register for Spec Editor");
            _toolTip.SetToolTip(
                btnPublishCatalog,
                "Generate Code + Deploy + PLANTREGISTERCUSTOMSCRIPTS (with pre-flight checks)");
            _toolTip.SetToolTip(
                btnTestCatalog,
        ...
        private void btnDeployCatalog_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                CatalogDeployResult result = CatalogDeployService.DeployToCustomScripts();
                bool registered = CatalogDeployService.TryQueueRegisterCustomScripts(
                ...
                {
                    MessageBox.Show(
                        CatalogDeployResult.DialogText(result.ScriptCount, registered),
                        "Deploy Catalog",
        ...
        private void btnPublishCatalog_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (!ConfirmPublishPreflight(preflight, out bool allowWarnings))
                    return;
                ...
                if (result.Success)
                {
                    ...
                    MessageBox.Show(
                        result.Message + Environment.NewLine + Environment.NewLine + result.PartFolder,
                        "Publish Catalog",
                        MessageBoxButtons.OK,
                        MessageBoxIcon.Information);
                }
        ...
        private void btnGenerateCode_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                lblStatus.Text =
                    $"Exported {exported.Count} file(s) → {package.FolderName}; " +
                    "ScriptGroup/variants synced — Deploy Catalog for Spec Editor";
