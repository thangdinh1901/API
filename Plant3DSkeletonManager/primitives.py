...
class Elbow(ShapeObject):
    ...
    def arc_inlet_position(self) -> tuple[float, float, float]:
        """Inlet port center on first leg (matches 90° LR catalog scripts)."""
        return self.centerX, 0.0, 0.0

    def arc_outlet_position(self) -> tuple[float, float, float]:
        """Outlet port at ARC3D connection point 1 (native when available)."""
        try:
            p = self.obj.pointAt(1)
            return float(p.x()), float(p.y()), float(p.z())
        except Exception:
            pass

        cx, cy, a = self.centerX, self.centerY, self.angle
        r = self.bend_radius
        if abs(a - 90.0) < 1e-6:
            return 0.0, cy, 0.0

        end_rad = math.radians(-90.0 - a)
        return cx + r * math.cos(end_rad), cy + r * math.sin(end_rad), 0.0

    def arc_outlet_direction(self) -> tuple[float, float, float]:
        """Unit outlet direction from ARC3D (pipe continuation on leg 2)."""
        try:
            d = self.obj.directionAt(1)
        ...
        if abs(self.angle - 90.0) < 1e-6:
            return 0.0, 1.0, 0.0

        rad = math.radians(180.0 - self.angle)
