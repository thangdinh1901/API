    ...
    public static class ProjectValidator
    {
        public static ValidationResult Validate(ValveProject project)
        {
            ...
            foreach (PrimitiveNode node in project.Parts)
            {
                ...
                {
                    if (PrimitiveParameterUnits.RequiresPositiveValue(node.Type, kv.Key))
                    {
                        if (kv.Value.Value <= 0)
                        {
                            result.AddError(
                                $"Node '{node.Name}': {PrimitiveParameterUnits.ValidationMessage(node.Type, kv.Key)}");
                        }
                    }
                    else if (kv.Value.Value < 0)
                    {
                        result.AddError(
                            $"Node '{node.Name}': {PrimitiveParameterUnits.ValidationMessage(node.Type, kv.Key)}");
                    }
                    ...
                    if (!string.IsNullOrWhiteSpace(kv.Value.Expression) &&
                        !IsIgnorableExpressionWarning(node, kv.Key, kv.Value.Expression) &&
        ...
        private static bool IsIgnorableExpressionWarning(
            PrimitiveNode node,
            string paramName,
            string expression)
        {
