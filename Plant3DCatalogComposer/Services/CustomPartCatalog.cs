    ...
    public static class CustomPartCatalog
    {
        ...
        public static CustomPartDefinition? FindById(string? id) =>
            string.IsNullOrWhiteSpace(id)
                ? null
                : Parts.FirstOrDefault(p => p.Id.Equals(id, StringComparison.OrdinalIgnoreCase));

        private static IReadOnlyList<CustomPartDefinition> Discover()
        {
            ...
            foreach (string dir in Directory.EnumerateDirectories(partsDir).OrderBy(Path.GetFileName))
            {
                string id = Path.GetFileName(dir);
                if (StandardCatalogGuard.IsSandboxDirectory(id))
                    continue;
                ...
                try
                {
                    ...
                    list.Add(new CustomPartDefinition
                    {
                        Role = role,
                        Id = id,
